...
class UserProf extends React.Component {
  ...
  componentDidMount() {
    ...
    if (this.props.currentUser.user_albums) {
      for (let i = 0; i < this.props.currentUser.user_albums.length; i++) {
        this.props.fetchAlbum(this.props.currentUser.user_albums[i].id);
      }
    }
  }
